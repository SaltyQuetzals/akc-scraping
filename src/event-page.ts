...
const extractClassInfo = (classCell: cheerio.Element) => {
  ...
  const classHref = /openWin\('(?<href>[^']+)/g.exec(classLink.attr('href')!)!
    .groups!.href;
...
const extractEntriesInfo = (entriesCell: cheerio.Element) => {
  ...
  const entriesData = $.text().replace(/\s+/g, ' ').trim();
  if (!entriesData) {
...
export {extractClassInfo, extractJudgeInfo, extractEntriesInfo};
